...
const Profile = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:7070/user/change-dp', formData, {
        headers: {
  ...
  }, [token, profileImage, accStatus, Posts]);
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:7070/user/profile/allposts', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchFriends = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:7070/user/allFriends', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const DeleteItem = async (postId, authToken) => {
    try {
      const response = await fetch(`http://localhost:7070/user/delete-post/${postId}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:7070/user/create-post', formData, {
        headers: {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'multipart/form-data', // Make sure to set the content type for FormData
        },
  ...
  const handleACstatus = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:7070/user/AccountStatus/${ele[0].checked}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="parent">
      <div className="left">
        ...
        <label className="switch">
          <input type="checkbox" checked={accStatus === "PUBLIC"} onClick={handleACstatus}/>
          ...
          <div className="field-content">
            <section className="profile-photo">
              <div className="user-profile-image">
                ...
                {/* <img src={userProfile.imageUrl} alt="Profile" className="profile-image" /> */}
                  {profileImage !== null? (
                    ...
                    <img src="https://icon-library.com/images/default-user-icon/default-user-icon-13.jpg" alt="Profile" className="profile-image" />
                  )}
                </label>
              ...
              <label  >
                  <input accept="image/png,image/jpeg, gif" className="hidden" id="profile_image_5585964168781" name="profile[image]" type="file" onChange={handleImageChange} />
                  <FaCamera className='uplader-icon'/>
              </label>
            </div>
      ...
      <div className="contentArea">
        ...
        <div className="contentBar">
          ...
          <button type="button"  onClick={() => showDiv("create-post")}>Create Post</button>
        </div>
        <div className="contentType">
          ...
          <div className="create-post hidden" id="create-post">
            <form className="preDiv"  onSubmit={handleSubmit}>
              ...
              <input type="file" name="uploadPost" id="uploadPost" onChange={postPreview }/>
                {/* <input type="text" name="PostDescription" id="PostDescription" onChange={(e)=>{setDescription(e.target.value)}}/>   */}
              <button type="submit" id="postSubmit" name="postSubmit" >Submit</button>
            </form>
          </div>
        </div>
      </div>
